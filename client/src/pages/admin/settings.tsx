...
type PlatformSettings = {
  ...
  rankRequirements: Record<
    string,
    { personalVolume: number; weakLegVolume: number }
  > | null;
...
function formatInterval(ms: number): string {
  ...
  if (seconds < 60)
    return `${seconds % 1 === 0 ? seconds : seconds.toFixed(1)} sec`;
...
export default function AdminSettings() {
  ...
  const testWebhookMutation = useMutation({
    mutationFn: () =>
      apiRequest("POST", api.admin.testWebhook.post.path).then(
        (r) => r.json() as Promise<{ success: boolean; message: string }>,
      ),
    onSuccess: (result) => {
      toast({
    ...
    onError: () =>
      toast({ title: "Failed to test webhook", variant: "destructive" }),
  });
  ...
  const [rankReqs, setRankReqs] = useState<
    Record<string, { personalVolume: number; weakLegVolume: number }>
  >({});
  ...
  useEffect(() => {
    if (settings) {
      setCompanyInfo({
        ...
        supportEmail:
          settings.companyInfo?.supportEmail ??
          "support@leadershieldfunding.com",
      });
      setRankReqs(
        settings.rankRequirements ?? {
  ...
  const sendWarningsMutation = useMutation({
    mutationFn: () =>
      apiRequest("POST", api.admin.subscriptions.sendWarnings.path).then(
        (r) =>
          r.json() as Promise<{ days: number; total: number; sent: number }>,
      ),
    onSuccess: (result) => {
      queryClient.invalidateQueries({
        queryKey: [api.admin.subscriptions.dueForWarning.path],
  ...
  function updateRankReq(
    rank: string,
    field: "personalVolume" | "weakLegVolume",
    value: string,
  ) {
    setRankReqs((prev) => ({
      ...prev,
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">
            Configure platform-wide settings and compensation parameters
          </p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Basic platform identity and contact details
                </CardDescription>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Minimum personal volume and weak-leg volume needed to qualify
                  for each rank
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateRankReq(rank, "weakLegVolume", e.target.value)
                          }
                        />
                      </div>
                    </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  How many days before a subscription expires to send the
                  warning email to the agent (1–90 days)
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-end gap-4">
                  <div className="space-y-1 w-48">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setExpiryWarningDays(parseInt(e.target.value) || 7)
                      }
                    />
                  </div>
                  <p className="text-sm text-gray-500 pb-2">
                    ...
                    <span className="font-medium text-gray-700">
                      {expiryWarningDays} day
                      {expiryWarningDays !== 1 ? "s" : ""}
                    </span>{" "}
                    before expiry
                  </p>
                </div>
                ...
                >
                  {warningPreviewLoading ? (
                    ...
                    <p className="text-sm text-gray-700">
                      ...
                      >
                        {warningPreview.count} subscription
                        {warningPreview.count !== 1 ? "s" : ""}
                      </Link>{" "}
                      would receive a warning email right now at the current
                      setting of{" "}
                      <span className="font-medium">
                        {warningPreview.days ?? expiryWarningDays} day
                        {(warningPreview.days ?? expiryWarningDays) !== 1
                          ? "s"
                          : ""}
                      </span>{" "}
                      before expiry.
                  ...
                  (!warningPreview || warningPreview.count === 0) ? (
                    <p
                      ...
                      data-testid="text-warning-preview-empty"
                    >
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Active 2026 commission model and holdback configuration. Edit
                  in the server config / compensation files to change.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="space-y-1">
                    <p className="font-medium text-gray-700">
                      MCA Producer Economics (2026)
                    </p>
                    <p className="text-gray-500">
                      Opening Agent Pool: 32.5% of gross funding commission
                    </p>
                    <p className="text-gray-500">
                      Independent agent keeps full 32.5%
                    </p>
                    <p className="text-gray-500">
                      Performance accelerators: up to +2.5%
                    </p>
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    <p className="font-medium text-gray-700">
                      Subscription Pools (first 90 days)
                    </p>
                    <p className="text-gray-500">
                      Premium (GF $497 / RGS $997 / Scale $1,497): Standard 45%
                      · Enhanced 50% · Elite 55%
                    </p>
                    <p className="text-gray-500">Starter ($149/mo): 25%–35%</p>
                    <p className="text-gray-500">
                      Accelerators: up to +5% (top rate 60% at Elite)
                    </p>
                    <p className="text-gray-500">
                      Residual (mo 13+): GF 10% · RGS/Scale 15% · Starter none
                    </p>
                  </div>
                  <div className="space-y-1">
                    <p className="font-medium text-gray-700">
                ...
                {webhookLoading ? (
                  ...
                  <>
                    {(!webhookStatus.secretStored ||
                      ...
                      webhookStatus.endpointActive === false) && (
                      <Alert
                        variant="destructive"
                        data-testid="alert-webhook-not-configured"
                      >
                        ...
                        <AlertDescription>
                          ...
                          Restart the app to re-initialize the Stripe webhook
                          endpoint.
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        {webhookStatus.secretStored ? (
                          <Badge
                            data-testid="badge-secret-stored"
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {webhookStatus.endpointActive === null ? (
                          <Badge
                            data-testid="badge-endpoint-unknown"
                            variant="secondary"
                          >
                            Unknown
                          </Badge>
                          ...
                          <Badge
                            data-testid="badge-endpoint-active"
                            className="bg-green-100 text-green-700 border-green-200"
                          >
                          ...
                          <Badge
                            data-testid="badge-endpoint-inactive"
                            variant="destructive"
                          >
                    ...
                    <div className="border-t pt-4 space-y-2">
                      ...
                      <p className="text-xs text-gray-500">
                        If automatic setup failed, paste the signing secret from
                        your Stripe webhook endpoint (starts with{" "}
                        <code className="font-mono">whsec_</code>) to recover
                        without restarting.
                      </p>
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            saveWebhookSecretMutation.mutate(
                              webhookSecret.trim(),
                            )
                          }
                          disabled={
                  ...
                  <div className="divide-y divide-gray-100">
                    {systemInfo.expiryCheckIntervalInvalid && (
                      <div className="pb-4">
                        <Alert
                          variant="destructive"
                          data-testid="alert-invalid-scheduler-interval"
                        >
                          <AlertTriangle className="h-4 w-4" />
                          <AlertTitle>
                            Invalid scheduler interval — using default
                          </AlertTitle>
                          <AlertDescription>
                            The configured{" "}
                            ...
                            <span
                              ...
                              data-testid="text-rejected-interval-value"
                            >
                              "{systemInfo.expiryCheckIntervalRejectedValue}"
                            </span>{" "}
                            was rejected because it is not a positive integer
                            (milliseconds). The scheduler has fallen back to the
                            default of{" "}
                            <span
                              className="font-semibold"
                              data-testid="text-fallback-interval"
                            >
                              {formatInterval(
                                systemInfo.expiryCheckIntervalDefaultMs,
                              )}{" "}
                              (
                              {systemInfo.expiryCheckIntervalDefaultMs.toLocaleString()}{" "}
                              ms)
                            </span>
                            . Update the value to fix how often expiry checks
                            run.
                          </AlertDescription>
                        </Alert>
                      </div>
                    ...
                    <div className="flex items-center justify-between gap-3 text-sm pb-4">
                      <div>
                        <p className="font-medium text-gray-700">
                          Subscription expiry check interval
                        </p>
                        <p className="text-gray-500">
                          How often the scheduler checks for expiring
                          subscriptions (
                          <code className="font-mono">
                            EXPIRY_CHECK_INTERVAL_MS
                          </code>
                      ...
                      >
                        ...
                        <span className="text-gray-400 ml-1">
                          ({systemInfo.expiryCheckIntervalMs.toLocaleString()}{" "}
                          ms)
                        </span>
                      </Badge>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3 text-sm py-4">
                      ...
                      <Badge
                        ...
                        className="font-mono whitespace-nowrap"
                      >
                        {systemInfo.expiryWarningDays} day
                        {systemInfo.expiryWarningDays !== 1 ? "s" : ""}
                      </Badge>
                    </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Trigger on-demand calculations
                </CardDescription>
              </CardHeader>
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await apiRequest(
                        "POST",
                        api.admin.commissions.calculate.path,
                      );
                      toast({ title: "Binary bonus calculation triggered" });
                    } catch {
                      toast({
                        title: "Failed to trigger calculation",
                        variant: "destructive",
                      });
                    }
                  }}
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      toast({
                        title: `Subscription commissions calculated: ${result.processed} processed`,
                      });
                    } catch {
                      toast({
                        title: "Failed to calculate subscription commissions",
                        variant: "destructive",
                      });
                    }
                  }}
