...
export function WaterfallBreakdown({ deal }: { deal: any }) {
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-lg font-bold">
            ${gbrAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
          </span>
        </div>
        <div className="space-y-2">
          <div className="text-sm font-semibold">
            Opening Agent Pool (32.5% = ${openingPool.toFixed(2)})
          </div>
          <p className="ml-4 text-sm text-muted-foreground">
            Independent agents keep the full 32.5%. Agency models (Small Agency,
            Leadership, Recruiting) allocate part of this pool as team overrides
            — producer + override always sums to 32.5%.
          </p>
        </div>
        <div className="flex items-center justify-between gap-2 text-sm">
          ...
          <span className="font-medium">
            up to ${acceleratorMax.toFixed(2)}
          </span>
        </div>
        <p className="text-xs text-muted-foreground">
          Subscription Attachment, Repeat Merchant, and volume accelerators
          stack to a 2.5% cap of the gross funding commission.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="text-sm font-semibold">
          MAC (30% of GBR = ${macTotal.toFixed(2)})
        </div>
      ...
      <div className="flex items-center justify-between gap-2 text-sm">
        ...
        <span className="font-medium">
          ${tfcMin.toFixed(2)} - ${tfcMax.toFixed(2)}
        </span>
      </div>
      <div className="flex items-center justify-between gap-2 text-sm">
        <span className="font-semibold">PICF (25-35% of GBR)</span>
        <span className="font-medium">
          ${picfMin.toFixed(2)} - ${picfMax.toFixed(2)}
        </span>
      </div>
      <div className="flex items-center justify-between gap-2 text-sm">
...
export default function AdminDeals() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(api.admin.deals.list.path, {
        credentials: "include",
      });
  ...
  const { data: agentsList } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        buildUrlWithQuery(api.admin.agents.list.path, undefined, {
          pageSize: 200,
        }),
        { credentials: "include" },
      );
  ...
  const updateDealMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: any }) => {
      const res = await apiRequest(
        "PATCH",
        buildUrl(api.admin.deals.update.path, { id }),
        data,
      );
      return res.json();
    },
  ...
  const totalVolume = deals.reduce(
    (sum: number, d: any) => sum + Number(d.loanAmount || 0),
    0,
  );
  const totalGbr = deals.reduce(
    (sum: number, d: any) => sum + Number(d.gbrAmount || 0),
    0,
  );
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "funded":
        return <Badge data-testid={`badge-status-${status}`}>Funded</Badge>;
      case "pending":
        return (
      ...
      <header className="flex items-center justify-between flex-wrap gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage deals, GBR waterfall breakdown, and fulfillment agent
            assignments.
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-3xl" data-testid="text-total-deals">
              {deals.length}
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle
              ...
              data-testid="text-total-gbr"
            >
              ${totalGbr.toLocaleString()}
            </CardTitle>
          </CardHeader>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-4">
          <div>
            ...
            <CardDescription>
              View and manage deal details with GBR waterfall
            </CardDescription>
          </div>
          <Select
            value={statusFilter}
            ...
            <TableBody>
              {isLoading ? (
                ...
                filteredDeals.map((deal: any) => {
                  ...
                  const heldAmount = dealHoldbacks.reduce(
                    (s: number, h: any) => s + Number(h.totalAmount || 0),
                    0,
                  );
                  return (
                    <TableRow key={deal.id} data-testid={`row-deal-${deal.id}`}>
                      <TableCell>
                        <div>
                          <p
                            ...
                            data-testid={`text-merchant-${deal.id}`}
                          >
                            {deal.merchantName}
                          </p>
                          <p className="text-xs text-muted-foreground">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs">
                            {deal.agent?.firstName?.[0]}
                            {deal.agent?.lastName?.[0]}
                          </div>
                      ...
                      <TableCell data-testid={`text-gbr-${deal.id}`}>
                        {deal.gbrAmount ? (
                          <span className="font-medium text-blue-600">
                            ${Number(deal.gbrAmount).toLocaleString()}
                          </span>
                        ) : (
                      ...
                      <TableCell>
                        {deal.fulfillmentAgent ? (
                          <span className="text-sm">
                            {deal.fulfillmentAgent.firstName}{" "}
                            {deal.fulfillmentAgent.lastName}
                          </span>
                        ) : deal.fulfillmentAgentId ? (
                          <span className="text-sm text-muted-foreground">
                            Agent #{deal.fulfillmentAgentId}
                          </span>
                        ) : (
                      ...
                      <TableCell className="text-muted-foreground text-sm">
                        {format(
                          new Date(deal.fundedAt || deal.createdAt),
                          "MMM d, yyyy",
                        )}
                      </TableCell>
                      <TableCell className="text-right">
                        <Dialog>
                          ...
                          <DialogContent className="max-w-lg">
                            <DialogHeader>
                              <DialogTitle data-testid="text-deal-detail-title">
                                Deal #{deal.id} - {deal.merchantName}
                              </DialogTitle>
                              <DialogDescription>
                                GBR waterfall breakdown and holdback details
                              </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-6">
                              ...
                              {dealHoldbacks.length > 0 && (
                                <div className="border-t pt-4">
                                  <h4 className="font-semibold mb-3">
                                    Holdbacks ({dealHoldbacks.length})
                                  </h4>
                                  <div className="space-y-2">
                              ...
                              <div className="border-t pt-4">
                                ...
                                <Select
                                  value={
                                    deal.fulfillmentAgentId?.toString() || ""
                                  }
                                  onValueChange={(val) => {
