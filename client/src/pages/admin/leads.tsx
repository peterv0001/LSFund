...
export default function AdminLeadsPage() {
  ...
  const assignMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await apiRequest("POST", "/api/admin/leads/assign", {
        leadIds,
        agentId,
      });
      return res.json();
  ...
  const uploadMutation = useMutation({
    mutationFn: async (leads: any[]) => {
      const res = await apiRequest("POST", "/api/admin/leads/upload", {
        leads,
      });
      return res.json();
    },
  ...
  const respondToRequestMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await apiRequest(
        "POST",
        `/api/admin/lead-requests/${id}/respond`,
        { status, leadsAssigned },
      );
      return res.json();
    },
    onSuccess: () => {
  ...
  const toggleSelectAll = () => {
    ...
    const unassignedIds = filteredLeads
      .filter((l) => !l.assignedAgentId)
      .map((l) => l.id);
      ...
      <header className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Upload, distribute, and track leads
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {selectedLeads.length > 0 && (
            <Button
              ...
              data-testid="button-assign-leads"
            >
                  ...
                  <Checkbox
                    checked={
                      ...
                      selectedLeads.length ===
                        filteredLeads?.filter((l) => !l.assignedAgentId).length
                    }
