...
export default function AdminPayoutsPage() {
  ...
  const [showMarkPaidDialog, setShowMarkPaidDialog] = useState<Payout | null>(
    null,
  );
  ...
  const { data: payoutsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(api.admin.payouts.list.path, {
        credentials: "include",
      });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`${api.admin.payouts.preview.path}?${params}`, {
        credentials: "include",
      });
      if (!res.ok) throw new Error("Failed to fetch");
  ...
  const createPayoutMutation = useMutation({
    ...
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to create payouts.",
        variant: "destructive",
      });
    },
  });
  ...
  const markPaidMutation = useMutation({
    mutationFn: async (payoutId: number) => {
      const res = await fetch(
        api.admin.payouts.markPaid.path.replace(":id", String(payoutId)),
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <Card>
          ...
          <CardContent>
            <p className="text-xs text-muted-foreground">
              ...
              {pendingPayouts.length !== 1 ? "s" : ""} waiting
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-xs text-muted-foreground">
              {completedPayouts.length} payout
              {completedPayouts.length !== 1 ? "s" : ""} completed
            </p>
          </CardContent>
        </Card>
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Select a period to calculate and create payouts for all agents
              with approved commissions.
            </DialogDescription>
          </DialogHeader>
            ...
            {preview && preview.length > 0 && (
              <div className="border rounded-lg overflow-hidden">
                <div className="bg-gray-50 px-4 py-2 font-medium text-sm">
                  {preview.length} agent{preview.length !== 1 ? "s" : ""} with
                  payable commissions
                </div>
                      ...
                      {preview.map((p) => (
                        <TableRow key={p.agentId}>
                          ...
                          <TableCell className="text-right font-mono font-semibold text-emerald-600">
                            $
                            {p.totalAmount.toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                            })}
                          </TableCell>
                        </TableRow>
          ...
          <DialogFooter>
            ...
            >
              ...
              Create {preview?.length || 0} Payout
              {preview?.length !== 1 ? "s" : ""}
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="flex justify-between mb-2">
                  ...
                  <span className="font-medium">
                    {showMarkPaidDialog.agent?.firstName}{" "}
                    {showMarkPaidDialog.agent?.lastName}
                  </span>
                </div>
                <div className="flex justify-between mb-2">
                  ...
                  <span className="font-mono font-semibold text-emerald-600">
                    $
                    {Number(showMarkPaidDialog.amount).toLocaleString(
                      undefined,
                      { minimumFractionDigits: 2 },
                    )}
                  </span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {showMarkPaidDialog.agent?.payoutEmail ||
                      showMarkPaidDialog.agent?.email}
                  </span>
                </div>
              </div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() =>
                showMarkPaidDialog &&
                markPaidMutation.mutate(showMarkPaidDialog.id)
              }
