  ...
  const markProcessedMutation = useMutation({
    mutationFn: async (id: number) => {
      const res = await apiRequest(
        "POST",
        `/api/admin/leads/${id}/ai-processed`,
      );
      return res.json();
    },
    onSuccess: () => {
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Leads flagged for AI-powered follow-up
            </p>
          </div>
        </div>
      </header>
      ...
      <div className="bg-white rounded-xl border shadow-sm">
        {isLoading ? (
          ...
          <div className="divide-y">
            {queue.map((lead) => (
              <div
                ...
                data-testid={`row-ai-lead-${lead.id}`}
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="mt-2 text-xs text-muted-foreground">
                      Submitted{" "}
                      {lead.aiFollowupRequestedAt
                        ? format(
                            new Date(lead.aiFollowupRequestedAt),
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-muted-foreground">
              No leads are currently awaiting AI follow-up.
            </p>
          </div>
        )}
      </div>
