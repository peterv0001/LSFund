...
export default function AdminCommissions() {
  ...
  const { data: holdbacksData, isLoading: holdbacksLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("/api/admin/holdbacks", {
        credentials: "include",
      });
      if (!res.ok) throw new Error("Failed to fetch");
      return res.json();
  ...
  const approveMutation = useMutation({
    mutationFn: async (id: number) => {
      const res = await fetch(
        buildUrl(api.admin.commissions.approve.path, { id }),
        {
          method: "POST",
          credentials: "include",
        },
  ...
  const approveAllMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: "Success",
        description: `${data.approved} commissions approved`,
      });
    },
  });
  ...
  const calculateMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: "Success",
        description: `Binary bonuses calculated for ${data.processed} agents`,
      });
    },
  });
  ...
  const subCalcMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: "Success",
        description: `Subscription commissions calculated for ${data.processed} subscriptions${skippedNote}`,
      });
    },
  });
  ...
  const releaseEligibleMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        title: "Success",
        description: `${data.released} holdbacks released`,
      });
    },
  });
  ...
  const heldHoldbacks = ((holdbacksData as any[]) || []).filter(
    (h: any) => h.status === "held" || h.status === "partially_released",
  );
  const totalHeld = heldHoldbacks.reduce(
    (s: number, h: any) =>
      s + Number(h.totalAmount || 0) - Number(h.releasedAmount || 0),
    0,
  );
    ...
    <AdminLayout>
      ...
      <Tabs defaultValue="commissions" className="space-y-6">
        <TabsList data-testid="tabs-commission-sections">
          ...
          <TabsTrigger value="holdbacks" data-testid="tab-holdbacks">
            Holdbacks
          </TabsTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle
                  className="text-3xl font-mono text-emerald-600"
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This will calculate binary bonuses for all qualifying
                        agents based on their leg volumes this week.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        ...
                        data-testid="button-confirm-binary"
                      >
                        Run Calculation
                      </AlertDialogAction>
                    </AlertDialogFooter>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This will approve {pendingCommissions?.length || 0}{" "}
                        commissions totaling ${totalPending.toLocaleString()}.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-lg">
                  Fulfillment Tier Rates
                </CardTitle>
                <CardDescription>
                  TFC percentage by fulfillment agent tier
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-wrap gap-4">
                  {Object.entries(
                    fulfillmentData.fulfillmentTierRates || {},
                  ).map(([tier, rate]) => (
                    ...
                    pendingCommissions.map((comm: any) => (
                      ...
                      >
                        <TableCell>
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs">
                              {comm.agent?.firstName?.[0]}
                              {comm.agent?.lastName?.[0]}
                            </div>
                            <div>
                        ...
                        <TableCell>
                          <Badge
                            variant="secondary"
                            className={getTypeBadgeColor(comm.type)}
                            data-testid={`badge-type-${comm.id}`}
                          >
                            {COMMISSION_TYPE_LABELS[comm.type] ||
                        ...
                        <TableCell
                          ...
                          data-testid={`text-amount-${comm.id}`}
                        >
        ...
        <TabsContent value="holdbacks" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle
                  className="text-3xl font-mono"
                  data-testid="text-holdback-count"
                >
                  {heldHoldbacks.length}
                </CardTitle>
              </CardHeader>
            </Card>
            <Card>
              ...
              <CardContent className="pt-6">
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button
                      className="w-full"
                      disabled={releaseEligibleMutation.isPending}
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>
                        Release Eligible Holdbacks?
                      </AlertDialogTitle>
                      <AlertDialogDescription>
                        This will release all holdbacks that have passed their
                        release date. Released funds will become available as
                        commissions.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        ...
                        data-testid="button-confirm-release-eligible"
                      >
                        Release Eligible
                      </AlertDialogAction>
                    </AlertDialogFooter>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                70% released at funding, 30% deferred 60-90 days
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                  ...
                  {holdbacksLoading ? (
                    ...
                    (holdbacksData as any[]).map((h: any) => (
                      <TableRow key={h.id} data-testid={`row-holdback-${h.id}`}>
                        ...
                        <TableCell className="font-medium">
                          ${Number(h.totalAmount).toFixed(2)}
                        </TableCell>
                        <TableCell className="text-emerald-600">
                          ${Number(h.releasedAmount || 0).toFixed(2)}
                        </TableCell>
                        <TableCell>
                          <Badge
                            variant="secondary"
                            className={HOLDBACK_STATUS_COLORS[h.status] || ""}
                            data-testid={`badge-holdback-status-${h.id}`}
                          >
                            ...
                            {(h.status === "held" ||
                              ...
                              <>
                                <Button
                                  ...
                                  variant="ghost"
                                  onClick={() =>
                                    releaseHoldbackMutation.mutate(h.id)
                                  }
                                ...
                                <Dialog>
                                  ...
                                  <DialogContent>
                                    <DialogHeader>
                                      ...
                                      <DialogDescription>
                                        Clawback holdback #{h.id} ($
                                        {Number(h.totalAmount).toFixed(2)}) for
                                        Deal #{h.dealId}
                                      </DialogDescription>
                                    </DialogHeader>
                                    <div className="space-y-4">
                                      <div>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">
                                          Schedule: 0-30 days = 100%, 31-90 days
                                          = 50%, after 90 = 0%
                                        </p>
                                      </div>
                                      <div>
                                        ...
                                        <Textarea
                                          ...
                                          onChange={(e) =>
                                            setClawbackReason(e.target.value)
                                          }
        ...
        <TabsContent value="subscriptions" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle
                  className="text-3xl font-mono text-emerald-600"
                  data-testid="text-active-sub-count"
                >
                  {
                    ((subscriptionsData as any[]) || []).filter(
              ...
              <CardContent className="pt-6">
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button
                      className="w-full"
                      disabled={subCalcMutation.isPending}
                      data-testid="button-calc-sub-commissions"
                    >
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This will calculate monthly subscription commissions for
                        all active subscriptions, applying the decay schedule.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                      <AlertDialogAction
                        onClick={() => subCalcMutation.mutate()}
                        data-testid="button-confirm-calc-sub"
                      >
                        Calculate
                      </AlertDialogAction>
                    </AlertDialogFooter>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Subscription commission rates decrease over time
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manage merchant subscriptions and track production compliance
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                      ...
                      return (
                        ...
                        >
                          <TableCell>
                            <div>
                              <p
                                ...
                                data-testid={`text-sub-merchant-${sub.id}`}
                              >
                                {sub.merchantName}
                              </p>
                              {sub.merchantEmail && (
                          ...
                          <TableCell>
                            <Badge
                              variant="outline"
                              data-testid={`badge-sub-tier-${sub.id}`}
                            >
